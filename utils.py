...
def get_search_params(search_api: str, search_api_config: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    if search_api == "exa":
        ...
        valid_params = ["max_characters", "num_results", "include_domains", "exclude_domains", "subpages"]
        ...
        valid_params = ["max_results", "search_depth", "include_domains", "exclude_domains", "include_raw_content"]
        for param in valid_params:
            if param in search_api_config:
                params_to_pass[param] = search_api_config[param]
...
def normalize_search_results(search_responses: List[Dict[str, Any]]) -> List[SearchResponse]:
...
async def tavily_search_async(search_queries: List[str], **kwargs) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for item in data.get("results", []):
                result = {
                    ...
                    "url": item.get("url", ""),
                    "content": item.get("snippets", [""])[0] if item.get("snippets") else "",
                    "score": 1.0,
                    "raw_content": None
                }
...
async def select_and_execute_search(search_api: str, query_list: List[str], params: Dict[str, Any]) -> List[SearchResponse]:
